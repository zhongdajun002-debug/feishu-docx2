...
feishu-docx2 --help        # list CLI commands
```
...
Authentication is handled via `feishu-docx2 config set` and `feishu-docx2 auth` (OAuth flow). Avoid committing tokens or generated output; keep samples in `docs/` or `tests/output/`.
